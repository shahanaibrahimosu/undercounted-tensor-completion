...
if dataset=="PPI":
	...
	F_list = [5,10,15,20]   # Rank of the tensor
	D = 35  # dimension of the detection feature vector
elif dataset=="Covid":
	K=3
	I = [43,80,60] # County latitude, County longitude, No of dates
	F_list = [15, 20]   # Rank of the tensor
...
no_of_trials = 1
fold_num = 5
no_of_BCD_iterations=100
no_of_epochs_theta=1
no_iteration_MM=1
no_inner_iter_MM=1
tol = 1e-6
#mu = 100 #regularization for features with no observation
...
print(observed_data.I_selected_indices)
						...
						for l_g,learning_rate_g in enumerate(learning_rate_list):
							...
							for b,batch_size in enumerate(batch_size_list):
								for m,mu in enumerate(mu_list):
									...
									L_copy =out['Lambda']
									file_L = open('Lambda_with_indices_'+str(k)+'.txt','w')
									for ii,ind1 in enumerate(I_selected_indices):
										for jj,ind2 in enumerate(J_selected_indices):
											for kk,ind3 in enumerate(K_selected_indices):
												file_L.write( str(ind1)+'\t'+str(ind2)+'\t'+str(ind3)+'\t'+str(L_copy[ii,jj,kk])+'\n' )
									file_L.close()	
									##################################Recording P values###################################
									print('#################################################################################')
									print('VALIDATION - UNCLE-TC : #trial = [{}/{}], #fold [{}/{}], rank={}, hidden_layer={},'.format(i, no_of_trials, k,fold_num,F, hidden_layer_g))
									print('hidden_units={},learning_rate={:.4f}, batch_size={}, mu={}'.format(hidden_unit_g,learning_rate_g, batch_size,mu))
...
if os.path.exists(result_dir+'final_results_std.csv'):
	os.remove(result_dir+'final_results_std.csv')
#############################################Final Results########################################################################
print('#################################################################################')
print('UNCLE_TC: rRMSE={:.4f}, MAPE={:.4f},F1 Score={:.4f},Hamming Loss={:.4f}'.format(np.std(metrics_UNCLE_TC[0]),\
			np.std(metrics_UNCLE_TC[1]),np.std(metrics_UNCLE_TC[2]),np.std(metrics_UNCLE_TC[3])))
print('UNCLE_TC_linear: rRMSE={:.4f}, MAPE={:.4f},F1 Score={:.4f},Hamming Loss={:.4f}'.format(np.std(metrics_UNCLE_TC_linear[0]),\
			np.std(metrics_UNCLE_TC_linear[1]),np.std(metrics_UNCLE_TC_linear[2]),np.std(metrics_UNCLE_TC_linear[3])))
print('NTF-KL:rRMSE={:.4f}, MAPE={:.4f},F1 Score={:.4f},Hamming Loss={:.4f}'.format(np.std(metrics_NTF_KL[0]),\
			np.std(metrics_NTF_KL[1]),np.std(metrics_NTF_KL[2]),np.std(metrics_NTF_KL[3])))
print('BPTF: rRMSE={:.4f}, MAPE={:.4f},F1 Score={:.4f},Hamming Loss={:.4f}'.format(np.std(metrics_BPTF[0]),\
			np.std(metrics_BPTF[1]),np.std(metrics_BPTF[2]),np.std(metrics_BPTF[3])))
print('HaLRTC: rRMSE={:.4f}, MAPE={:.4f},F1 Score={:.4f},Hamming Loss={:.4f}'.format(np.std(metrics_HaLRTC[0]),\
			np.std(metrics_HaLRTC[1]),np.std(metrics_HaLRTC[2]),np.std(metrics_HaLRTC[3])))
print('NTF-LS: rRMSE={:.4f}, MAPE={:.4f},F1 Score={:.4f},Hamming Loss={:.4f}'.format(np.std(metrics_NTF_LS[0]),\
			np.std(metrics_NTF_LS[1]),np.std(metrics_NTF_LS[2]),np.std(metrics_NTF_LS[3])))
print('NTF-Tucker-LS: rRMSE={:.4f}, MAPE={:.4f},F1 Score={:.4f},Hamming Loss={:.4f}'.format(np.std(metrics_NTF_Tucker_LS[0]),\
			np.std(metrics_NTF_Tucker_LS[1]),np.std(metrics_NTF_Tucker_LS[2]),np.std(metrics_NTF_Tucker_LS[3])))
print('#################################################################################') 
result = [['UNCLE_TC',np.std(metrics_UNCLE_TC[0]),\
			np.std(metrics_UNCLE_TC[1]),np.std(metrics_UNCLE_TC[2]),np.std(metrics_UNCLE_TC[3])],
		  ['UNCLE_TC_linear',np.std(metrics_UNCLE_TC_linear[0]),np.std(metrics_UNCLE_TC_linear[1]),\
		   np.std(metrics_UNCLE_TC_linear[2]),np.std(metrics_UNCLE_TC_linear[3])],
		  ['NTF-KL',np.std(metrics_NTF_KL[0]),\
			np.std(metrics_NTF_KL[1]),np.std(metrics_NTF_KL[2]),np.std(metrics_NTF_KL[3])],
		  ['BPTF',np.std(metrics_BPTF[0]),\
			np.std(metrics_BPTF[1]),np.std(metrics_BPTF[2]),np.std(metrics_BPTF[3])],
		  ['HaLRTC',np.std(metrics_HaLRTC[0]),\
			np.std(metrics_HaLRTC[1]),np.std(metrics_HaLRTC[2]),np.std(metrics_HaLRTC[3])],
		  ['NTF-LS',np.std(metrics_NTF_LS[0]),\
			np.std(metrics_NTF_LS[1]),np.std(metrics_NTF_LS[2]),np.std(metrics_NTF_LS[3])],
		  ['NTF-Tucker-LS',np.std(metrics_NTF_Tucker_LS[0]),\
			np.std(metrics_NTF_Tucker_LS[1]),np.std(metrics_NTF_Tucker_LS[2]),np.std(metrics_NTF_Tucker_LS[3])]	
		 ]
Final_results = pd.DataFrame(result, columns=['Algorithms','rRMSE','MAPE','F1 Score', 'Hamming Loss'])	  
Final_results.to_csv(result_dir+'final_results_std.csv', index=False)
