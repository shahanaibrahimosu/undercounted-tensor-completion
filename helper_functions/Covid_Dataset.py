...
class Covid_Dataset:
	def __init__(self,network_parameters):
		self.obs_file ="dataset/Covid/obs.txt"
		self.feature_file_lam="dataset/Covid/lam_fea.txt"
		self.feature_file_det="dataset/Covid/det_fea.txt"
		self.I_orig = 43
		self.J_orig = 80
		self.K_orig = 471
	...
	def load_data(self,data_threshold):
		...
		Y_all[data[:,0]-1,data[:,1]-1,data[:,2]-1] = data[:,3].float()
		...
		Y_mid1 = Y_mid[plant_sum_indices[0:self.I],:,:]
		dates_sum = torch.nansum(Y_mid1,dim=[0,1])
		dates_sum_sorted, dates_sum_indices = torch.sort(dates_sum,descending=True)
		self.Y = Y_mid1[:,:,torch.arange(366,426)]
		#self.Y = Y_mid1[:,:,dates_sum_indices[0:self.K]]
		#plant_sum_indices = torch.arange(0,self.I)
		#pollinator_sum_indices = torch.arange(0,self.J)
		#dates_sum_indices = torch.arange(0,self.K)
		#self.Y=Y_all[:,:,torch.arange(366,426)]
		
		self.I_selected_indices = plant_sum_indices[0:self.I]
		self.J_selected_indices = pollinator_sum_indices[0:self.J]
		#self.K_selected_indices = dates_sum_indices[0:self.K]
		self.K_selected_indices = torch.arange(366,426)
		...
		Z2_indices = np.asarray(data2[:,0:3],dtype=np.long)-1
		#Z2_data	= data2[:,3:3+self.R2]#data[:,3+self.R1:3+self.R1+self.R2]
		xx=list(range(3,24))+list(range(26,29))
		Z2_data	= data2[:,xx]#data[:,3+self.R1:3+self.R1+self.R2]
		...
		for count,indices in enumerate(Z2_indices):
			...
			if torch.sum(i==plant_sum_indices[0:self.I])!=0 and torch.sum(j==pollinator_sum_indices[0:self.J])!=0\
							   and torch.sum(k==dates_sum_indices[0:self.K])!=0:
				i_index =torch.nonzero(i==plant_sum_indices[0:self.I])
				j_index = torch.nonzero(j==pollinator_sum_indices[0:self.J])
				k_index = torch.nonzero(k==dates_sum_indices[0:self.K])
				#Normalize the data
				data_Z2 = torch.tensor(Z2_data[count,:])
				#data_Z2 = (data_Z2-torch.min(data_Z2))/(torch.max(data_Z2)-torch.min(data_Z2))
				Z2_t[i_index,j_index,k_index,:]=data_Z2#torch.tensor(Z2_data[count,:])
		self.Z2=Z2_t.view(self.I*self.J*self.K,self.R2)
		print("Z data")
		print(torch.sum(self.Z2,dim=0))
		print(torch.sum(self.Z2,dim=1))
		print(self.Z2[0,:])
		#normalize the features
		#for r in range(self.R1):
		#	self.Z1[:,r] = self.Z1[:,r] /torch.max(self.Z1[:,r]  )
		for r in range(self.R2):
			#self.Z2[:,r]  = self.Z2[:,r] /torch.max(self.Z2[:,r]  )
